...
{
    public class CreditSuccessHandler : IEventHandler<CreditSuccessEvent>
    {
        private readonly ILogger<CreditSuccessEvent> _logger;
        private readonly IPaymentsRepository _paymentRepository;
        private readonly IOutboxRepository _outboxRepository;

        public CreditSuccessHandler(ILogger<CreditSuccessEvent> logger, IPaymentsRepository paymentRepository, IOutboxRepository outboxRepository)
        {
            _logger = logger;
        ...
        public async Task HandleAsync(CreditSuccessEvent @event, CancellationToken cancellationToken)
        {
            _logger.LogInformation("Processing CreditSuccessEvent [TransactionId={TransactionId}, UTR={UTR}]", @event.TransactionId, @event.Utr);
